//---------------------------------------------------------------------------------
// Project: Blink TM4C BIOS Using Task/Sem Dynamically (MEM) (STARTER)
// Author: Eric Wilbur
// Date: June 2014
//
...
//       from the peripheral - it does NOT cleabr the CPU interrupt flag - that
...
void main(void)
{
	...
	ledToggleSem = Semaphore_create(0, NULL, NULL);					// create ledToggleSem Semaphore
...
void ledToggle(void)
{
	...
	while(1)
	{
		Semaphore_pend(ledToggleSem, BIOS_WAIT_FOREVER);				// wait for Sem from ISR
...
void Timer_ISR(void)
{
	...
	Semaphore_post(ledToggleSem);										// post LEDSwi
}
