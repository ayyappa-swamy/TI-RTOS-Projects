//---------------------------------------------------------------------------------
// Project: Blink TM4C BIOS Using Mailbox/Queue (STARTER)
// Author: Eric Wilbur
// Date: June 2014
//
...
//------------------------------
//		Mailbox_post (LEDMbx, &msg, BIOS_WAIT_FOREVER);			// post msg containing LED state into the MAILBOX


//------------------------------
// QUEUE CODE follows...
//------------------------------
		Queue_put(LED_Que, (Queue_Elem*)msgp);						// pass pointer to Message object via LED_Queue
...
// MAILBOX CODE follows...
//------------------------------
//		Mailbox_pend(LEDMbx, &msg, BIOS_WAIT_FOREVER);									// wait/block until post of msg, get msg.val
...
//------------------------------
		...
		msgp = Queue_get(LED_Que);														// read contents of queue to get value of LED state
...
//		if (msg.val)																// MAILBOX "if" - msg.val contains LED state
		...
		{
			GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0|GPIO_PIN_4, 0x11);		// turn LED on
		}
		else
		{
			GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0|GPIO_PIN_4, 0x00);		// turn LED off
		}
